...
const loader = createCachedLoader(createIndexPaginationLoader({
    limit: 30,
    loader: (offset, limit) => API.getDropsByAdmin({ query: { type: "TAKEDOWN_REQUESTED", _offset: offset, _limit: limit } }).then(stupidErrorAlert),
}));

createPage(
    {
        route: createRoute({
            path: "/admin?list=takedown",
            events: {
                onLazyInit: async () => {
                    await loader.next();
                },
    ...
    Content(
        ...
        Box(loader.hasMore.map((hasMore) =>
            hasMore
                ...
                : Label("No more takedown requests")
        )),
    ),
);
